...
function onDeleteContactClickHandler(id) {
  Api.deleteById(state.token, id).then(() => loadContacts());
}

function onEditContactClickHandler(contact) {
  Api.editContact(state.token, contact).then(() => {
    loadContacts();
  });
}
...
function onAddContactClickHandler(contact) {
  Api.addContact(state.token, contact)
    .then(() => {
      loadContacts();
    })
    .catch((error) => {
...
function showError(error) {
  ...
  if (!div) {
    document
      ...
      .insertAdjacentHTML("afterbegin", `<div class="alert"></div>`);
